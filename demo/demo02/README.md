...
## 无状态函数式组件
官方指出：`在大部分React组件中，大部分组件被写成无状态组件，通过简单组合可以构建成其他的组件等；这种通过多个简单然后合并成一个大应用的设计模式被提倡。``

无状态函数式组件形式上表现为只带有一个`render`方法的组件类，通过函数形式或者ES6 arrow function的形式在创建，并且该组件是无`state`状态的。
```
